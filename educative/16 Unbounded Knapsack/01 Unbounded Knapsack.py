...
def solve_knapsack_dp(profits, weights, capacity):
    ...
    return solve_knapsack_recursive_dp(dp, profits, weights, capacity, 0)


def solve_knapsack_recursive_dp(dp, profits, weights, capacity, currentIndex):
    n = len(profits)
    # base checks
    if capacity <= 0 or n == 0 or len(weights) != n or currentIndex >= n:
        return 0
    ...
    if dp[currentIndex][capacity] == -1:
        ...
        profit1 = 0
        if weights[currentIndex] <= capacity:
            profit1 = profits[currentIndex] + solve_knapsack_recursive_dp(
                dp, profits, weights, capacity - weights[currentIndex], currentIndex)

        # recursive call after excluding the element at the currentIndex
        profit2 = solve_knapsack_recursive_dp(
